...
class SudokuPuzzle(Puzzle):
    ...
    def fail_fast(self) -> bool:
        ...
        for r in range(n):
            ...
            #         if self._fail_fast_helper(r, c):
            #             return True
            if self._fail_fast_iterate_for_a_row(r):
                return True
        return False

    def _fail_fast_helper(self, r: int, c: int) -> bool:
        """ Returns True if the there is a possible solution at point (r, c).
        If there is no possible solution, returns false.
        """
        symbol_set = self._symbol_set
        row = self._row_set(r)
        column = self._column_set(c)
        square = self._subsquare_set(r, c)
        solution_comparison = row | column | square
        # remove the empty
        solution_comparison.remove(EMPTY_CELL)
        if solution_comparison == symbol_set:
            return True
        return False

    def _fail_fast_iterate_for_a_row(self, r: int) -> bool:
        """Iterates over members of a row and checks for us, returns True, if
        all empty cells have valid points. Else returns false.
        """
        grid, n = self._grid, self._n
        for c in range(n):
            # checks if grid empty
            if grid[r][c] == EMPTY_CELL:
                if self._fail_fast_helper(r, c):
                    return True
        return False
    ...
    def has_unique_solution(self) -> bool:
        ...
        seen = set()
        seen.add(solved_puzzle[-1].__str__())
        if solver.solve(puzzle, seen) == []:
            return True
        return False
