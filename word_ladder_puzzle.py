...
class WordLadderPuzzle(Puzzle):
    ...
    def extensions(self) -> List[WordLadderPuzzle]:
        ...
        >>> wl0 = WordLadderPuzzle("come", "done", {"come", "con", "done"})
            ...
            if len(word) == len(self.from_word):
                for i in range(len(self.from_word)):
                    if self.from_word[i] != word[i]:
                        counter += 1
                if counter == 1:
                    extension_lst.append(WordLadderPuzzle(word, target, word_set))
    ...
    def get_difficulty(self) -> str:
        ...
        if self.from_word == self.to_word:
            ...
            solver1 = BfsSolver()
            path1 = solver1.solve(self)
            if len(path1) == 3:
                return EASY
            elif len(path1) == 1 or len(path1) == 2:
                return TRIVIAL
            elif len(path1) <= 5:
                return MEDIUM
            elif len(path1) > 5:
